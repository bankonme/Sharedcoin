...
public class BitcoinScript {
    ...
    public static BitcoinScript createSimpleOutBitoinScript(BitcoinAddress address) throws Exception {
        ...
        if (address.getVersion() == 0) {
            ...
            throw new Exception("Bitcoin address: " + address + " version "  + address.getVersion() + " not supported yet");
        }
    }
    ...
    public BitcoinScript(byte[] bytes) throws Exception {
        ...
        this.program = bytes;
        this.chunks = new ArrayList<>();  // Arbitrary choice of initial size.
    ...
    public int getInType() {
        if (this.chunks.size() == 1) {
            ...
            return ScriptInTypeAddress;
                ...
                this.chunks.get(0).length > 1 &&
                (this.chunks.get(1).length == 65 || this.chunks.get(1).length == 33)) {
        ...
        try {
            switch (this.getOutType()) {
                ...
                case ScriptOutTypeP2SH:
                    return new BitcoinAddress(new Hash(this.chunks.get(1)), (short) 5);
    ...
    public List<BitcoinAddress> extractAddresses() throws AddressFormatException {
        List<BitcoinAddress> addresses = new ArrayList<>();

        switch (this.getOutType()) {
            case ScriptOutTypeAddress:
                addresses.add(new BitcoinAddress(new Hash(this.chunks.get(2)), (short) 0));
                break;
            case ScriptOutTypePubKey:
                addresses.add(new BitcoinAddress(new Hash(Utils.sha256hash160(this.chunks.get(0))), (short) 0));
                break;
            case ScriptOutTypeP2SH:
                addresses.add(new BitcoinAddress(new Hash(this.chunks.get(1)), (short) 5));
                break;
            case ScriptOutTypeMultiSig:
                int n = unsignedByteToInt(this.chunks.get(this.chunks.size()-2)[0]) - OP_1 + 1;

                for (int ii = 0; ii < n; ++ii) {
                    addresses.add(new BitcoinAddress(new Hash(Utils.sha256hash160((this.chunks.get(this.chunks.size()-3-ii)))), (short)0));
                }
                break;
        }
        ...
        try {
            for (byte[] chunk : chunks) {
                ...
                if (chunk.length == 1) {
                    ...
                    try {
                        op_name = op_names[unsignedByteToInt(chunk[0])];
                    } catch(Exception e) { }

                    if (op_name != null) {
                        result += " " + op_name;
                    } else {
                        result += " " + new String(Hex.encode(chunk));
                    }
                } else {
                    result += " " + new String(Hex.encode(chunk));
                }
    ...
    public String getHexDecodedString() {
        try {
            return new String(getProgram());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getHexString() {
        return new String(Hex.encode(getProgram()));
    }

    public boolean IsPushOnly() throws Exception {
        cursor = 0;
        int length = program.length;
        while (cursor <  length) {
            int opcode = readByte();
            ...
            if (opcode > OP_0 && opcode < OP_PUSHDATA1) {
                ...
                // Read a uint32, then read that many bytes of data.
                throw new ScriptException("PUSHDATA4: Unimplemented");
            }
        }
    ...
    //Can only repair negative r & s
    public static boolean RepairNonCanonicalInput(TransactionInput input) throws ScriptException {

        Script inputScript = input.getScriptSig();
        byte[] vchSig = inputScript.getSignature();
        int nLenR = vchSig[3];
...

        {
            ...
            if ((vchSig[n + 0] & 0x80) > 0) {
                //Repair Non-canonical signature: R value negative;
                byte[] vchSig2 = new byte[vchSig.length + 1];

                //Copy first bytes
                System.arraycopy(vchSig, 0, vchSig2, 0, n);

                //Append zero byte
                vchSig2[n + 0] = 0;

                //Copy rest
                System.arraycopy(vchSig, n, vchSig2, n + 1, vchSig.length - n);
...

        {
            ...
            if ((vchSig[n + 0] & 0x80) > 0) {
                //Repair Non-canonical signature: S value negative;
        ...
        input.scriptBytes = Script.createInputScript(vchSig, inputScript.getPubKey());
        input.scriptSig = null;

        return IsCanonicalSignature(vchSig);
    }
    ...
    public boolean HasCanonicalPushes() throws Exception {
        cursor = 0;
        int length = program.length;
        while (cursor <  length) {
            int opcode = readByte();
            ...
            if (opcode > OP_0 && opcode < OP_PUSHDATA1) {
                ...
                if (data.length == 1 && data[0] <= 16)
                    throw new ScriptException("Could have used an OP_n code, rather than a 1-byte push.");
            } else if (opcode == OP_PUSHDATA1) {
                int len = readByte();
                byte[] data = getData(len);
                ...
                if (data.length < OP_PUSHDATA1)
                    throw new ScriptException("Could have used a normal n-byte push, rather than OP_PUSHDATA1.");
                ...
                if (data.length <= 0xFF)
                    throw new ScriptException("Could have used an OP_PUSHDATA1.");
            } else if (opcode == OP_PUSHDATA4) {
